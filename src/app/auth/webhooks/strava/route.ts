...
export async function handler(request: Request) {
  ...
  if (request.method === "POST") {
    ...
    const { object_type, aspect_type, object_id, owner_id, updates } =
      eventData;
    ...
    try {
      ...
      if (Date.now() / 1000 > stravaTokenData.expires_at) {
        console.log(
          "Token expired, refreshing token for athlete ID:",
          owner_id
        );
        stravaTokenData = await refreshStravaToken(
          stravaTokenData.refresh_token
        );
      ...
      if (
        ...
        // Check if this activity should be added to any map
  ...
  // Return a method not allowed response if the request method is not GET or POST
